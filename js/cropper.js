...
let cropState = {
  ...
  currentUrl: null,                 // 当前 ObjectURL，方便关闭时 revoke
};
...
function openCropper(file, onDone) {
  ...
  // 上一次的 URL 还在就先 revoke，别让反复裁剪时 blob URL 越积越多
  if (cropState.currentUrl) URL.revokeObjectURL(cropState.currentUrl);

  cropState.file = file;
  cropState.onDone = onDone;

  const url = URL.createObjectURL(file);
  cropState.currentUrl = url;
  ...
  img.onerror = function () {
    ...
    if (cropState.currentUrl === url) cropState.currentUrl = null;
  };
  img.src = url;
}
...
function closeCropper() {
  ...
  if (cropState.currentUrl) {
    URL.revokeObjectURL(cropState.currentUrl);
    cropState.currentUrl = null;
  }
}
...
/* ===============================
   自动裁剪（不弹窗）
   EPUB / 网页导入来的封面走这里：居中裁成 3:4 直接填进封面框，
   不满意再点「重新裁剪」手动调。
================================ */
function autoCrop3x4(source) {
  return new Promise(function (resolve, reject) {
    ...
    img.onload = function () {
      ...
      if (!nw || !nh) {
        URL.revokeObjectURL(url);
        reject(new Error("封面尺寸读不出来"));
        return;
      }

      // 在原图里取最大的 3:4 居中区域
      let sw = nw;
      let sh = nw / CROP_RATIO;
      if (sh > nh) {
        sh = nh;
        sw = nh * CROP_RATIO;
      }
      ...
      ctx.fillStyle = "#ffffff";
      ctx.fillRect(0, 0, OUT_WIDTH, OUT_HEIGHT);
      ctx.drawImage(img, (nw - sw) / 2, (nh - sh) / 2, sw, sh, 0, 0, OUT_WIDTH, OUT_HEIGHT);

      canvas.toBlob(
        function (blob) {
          URL.revokeObjectURL(url);
          blob ? resolve(blob) : reject(new Error("封面转换失败"));
        },
        "image/jpeg",
        0.92
      );
...
window.autoCrop3x4 = autoCrop3x4;
