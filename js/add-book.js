...
function pickMetaFile() {
  ...
  input.onchange = async function (e) {
    ...
    try {
      ...
      setImportStatus(meta.note || "读取完成，已把文件加入附件");
    } catch (err) {
      console.error(err);
...
async function applyWebData(data, url) {
  ...
  if (data.coverUrl && !coverBlob) {
    const blob = await fetchCoverBlob(data.coverUrl, url);
    if (blob) {
      lastCoverSource = blob;
      openCropper(blob, applyCropped);
    } else {
      setImportStatus("封面图跨域拿不到，请手动上传封面");
    }
  }
}
...
function renderHearts() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    heart.addEventListener("mousemove", function (e) {
      ...
      let fraction = (e.clientX - rect.left) / rect.width;
      fraction = Math.max(0.1, Math.min(1, Math.round(fraction * 20) / 20));
      paintHearts(i + fraction);
    });

    heart.addEventListener("mouseleave", () => paintHearts());
