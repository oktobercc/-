...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (editingId) {
    // 有些浏览器扩展会在页面上克隆 / 重排 DOM，导致 id 消失。
    // 找不到就跳过标题改字，别让整个初始化流程中断 —— 后面 loadExisting 才是关键
    const heading = document.getElementById("page-heading");
    const saveBtn = document.getElementById("save-btn");
    if (heading) heading.innerText = "编辑书籍";
...
/** 从网盘挑一个文件当附件。下载完就是个普通 File，
    后面走的路和本地选文件完全一样（EPUB 照样自动读元数据） */
async function pickFromNetdisk() {
  if (!window.Netdisk) {
    alert("网盘模块没加载");
    return;
  }
  if (!window.Auth || !Auth.loggedIn()) {
    alert("先去「关于我 → 账号」登录，再绑定网盘。");
    return;
  }

  try {
    const file = await Netdisk.pickFile();
    if (file) addFileAttachment(file);
  } catch (e) {
    alert("从网盘导入失败：" + e.message);
  }
}
...
================================ */
/* 心形用内联 SVG，不用 ♥ 字符：
   手机上 U+2665 会被系统按彩色 emoji 渲染，CSS 的 color 管不着它，
   于是底色和填充色都是那颗红心，滑动时看不出任何变化。
   SVG 的 fill 取 currentColor，颜色才真正受控。 */
const HEART_SVG =
...
function renderHearts() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    heart.innerHTML =
      ...
      '<span class="heart-fill"><span>' + HEART_SVG + "</span></span>";
...
async function submitBook(event) {
  ...
  // 找按钮的顺序：id → 事件源 → 按 onclick 属性兜底。
  // 找不到也不影响保存本身，只是不能显示「保存中…」状态而已
  const saveBtn =
    document.getElementById("save-btn") ||
    (event && (event.currentTarget || event.target)) ||
    document.querySelector('button[onclick*="submitBook"]');
