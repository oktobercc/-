...
/* 下载 / 打开私密书籍时要输的口令。
   注意：因为这是纯前端应用，任何 JS 变量都是可读的 —— 这个口令
   只能挡「拿到手机、随手翻两下」的场景，不是真正的加密保护。
   要真正的保护，应该把附件本身加密后再写 IndexedDB。
   口令存 localStorage，首次为空时会引导用户设置。 */
const DEFAULT_PASSWORD = "0025"; // 老版本硬编码值，作为迁移期的兼容口令
function getDownloadPassword() {
  const stored = localStorage.getItem("downloadPassword");
  return stored || DEFAULT_PASSWORD;
}
function setDownloadPassword(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    localStorage.removeItem("downloadPassword");
  } else {
    localStorage.setItem("downloadPassword", trimmed);
  }
}
...
/** 稳定序列化：递归对对象的 key 排序，避免云端返回和本地写入因
    键的插入顺序不同而被误判为「内容变了」，从而无谓地盖时间戳 */
 ...
 *   preserveTimestamps=true 时不动书本身的 updatedAt，用于「从备份恢复」这类
 *   场景 —— 恢复时如果盖了新时间戳，云同步会把老版本推回覆盖云端更新的内容。
 */
function saveBooks(books, options) {
  const preserveTimestamps = options && options.preserveTimestamps;

  if (!preserveTimestamps) {
    // 谁的内容变了就给谁盖一个新时间戳，云同步靠它判断新旧
    const previous = {};
    try {
      JSON.parse(localStorage.getItem("books") || "[]").forEach((b) => {
        previous[String(b.id)] = bookBody(b);
      });
    } catch (e) {
      /* 旧数据坏了就当全是新的 */
    }
    ...
    const now = Date.now();
    books.forEach((book) => {
      if (!book.updatedAt || previous[String(book.id)] !== bookBody(book)) {
        book.updatedAt = now;
      }
    });
  }
  ...
  // js/cloud.js 挂上来的钩子：本地一改就安排一次同步
  if (typeof window.onBooksSaved === "function") {
    try {
      window.onBooksSaved(books);
    } catch (e) {
      console.warn("同步钩子出错", e);
    }
  }
}
...
/* ===============================
   删除记录（墓碑）
   只记 id 和时间：本机删了的书，同步时才好让另一台也删掉
================================ */
function getDeletions() {
  try {
    return JSON.parse(localStorage.getItem("bookDeletions") || "[]");
  } catch (e) {
    return [];
  }
}
...
function recordDeletion(id) {
  const list = getDeletions().filter((d) => String(d.id) !== String(id));
  list.push({ id: id, at: Date.now() });
  localStorage.setItem("bookDeletions", JSON.stringify(list.slice(-500)));
}

function dropDeletions(ids) {
  const drop = ids.map(String);
  const list = getDeletions().filter((d) => drop.indexOf(String(d.id)) < 0);
  localStorage.setItem("bookDeletions", JSON.stringify(list));
}
...
function getAllCovers() {
  return withStore(COVER_STORE_NAME, "readonly", (store) => ({ __req: store.getAll() }));
}
...
function getAllAssets() {
  return withStore(ASSET_STORE_NAME, "readonly", (store) => ({ __req: store.getAll() }));
}
...
async function deleteBook(id, options) {
  let books = getBooks();
  books = books.filter((b) => String(b.id) !== String(id));
  saveBooks(books);

  // 从云端同步下来的删除不用再记一次，否则会来回推
  if (!(options && options.silent)) recordDeletion(id);
...
// 兼容旧代码：window.DOWNLOAD_PASSWORD 变成动态取值
Object.defineProperty(window, "DOWNLOAD_PASSWORD", {
  get: getDownloadPassword,
  configurable: true,
});
window.getDownloadPassword = getDownloadPassword;
window.setDownloadPassword = setDownloadPassword;
window.getBooks = getBooks;
window.saveBooks = saveBooks;
window.getDeletions = getDeletions;
window.recordDeletion = recordDeletion;
window.dropDeletions = dropDeletions;
...
window.getAllCovers = getAllCovers;
