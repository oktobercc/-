...
function readTxtMeta(file) {
  return new Promise((resolve) => {
    ...
    reader.onload = function (e) {
      // 中文圈的 TXT 大量是 GBK。先按 UTF-8 严格解一次，
      // 通不过或几乎没中文就退回 GBK，尽量别读成一片乱码
      const buffer = e.target.result;
      const bytes = new Uint8Array(buffer);
      let head = decodeSmart(bytes);
    ...
    reader.readAsArrayBuffer(file.slice(0, 20000));
  });
}

/** 先严格 UTF-8，通不过或几乎没中文就当 GBK 再试 */
function decodeSmart(bytes) {
  try {
    const strict = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
    const cjk = (strict.match(/[\u4e00-\u9fa5]/g) || []).length;
    // 有一定量中文就认为它是 UTF-8；纯 ASCII 也认为 UTF-8
    if (cjk > 0 || !/[\x80-\xff]/.test(strict)) return strict;
  } catch (e) {
    /* fatal 抛错 = 不是合法 UTF-8，走 GBK */
  }
  try {
