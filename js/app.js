...
function renderBooks() {
  ...
  if (window.ShelfActions) ShelfActions.syncAfterRender();
}

function createBookCard(book) {
  const div = document.createElement("div");
  div.className = "book-card" + (book.private ? " is-private" : "");
  div.setAttribute("data-id", book.id);

  div.onclick = function (e) {
    if (e.target.closest(".card-menu-btn")) return;

    // 长按刚选完之后系统补发的那一下 click 要先吞掉，否则会立刻取消选中
    if (window.ShelfActions && ShelfActions.clickSuppressed()) return;

    // 多选模式下点卡片是勾选，不是打开
    if (window.ShelfActions && ShelfActions.isSelecting()) {
      ShelfActions.toggle(book.id);
      return;
    }
  ...
  // 只允许几个安全的 URL 形态；其余一律回默认封面，
  // 防止恶意备份 .json 里塞 "><script> 之类
  const rawCover = book.cover && book.cover !== "custom" ? book.cover : "assets/default-cover.jpg";
  const coverSrc = escapeAttr(safeImageSrc(rawCover));

  div.innerHTML = `
    <div class="book-cover-wrapper">
      <img class="book-cover" src="${coverSrc}" alt="${escapeAttr(book.title || "未命名")}的封面">
      ${book.private ? '<span class="private-badge" title="私密书籍">🔒</span>' : ""}
    </div>

    <button class="card-menu-btn" type="button" aria-label="更多操作" title="更多操作">···</button>
  ...
  if (window.ShelfActions) ShelfActions.bindCardActions(div, book);
  return div;
}
...
function openBook(id) {
  ...
  if (window.ShelfActions && !ShelfActions.allowOpen(book)) return;

  location.href = "book-detail.html?id=" + encodeURIComponent(id);
}

/** 数据变了之后统一走这里重画书架（shelf-menu.js 也会调） */
...
async function renderGallery() {
  ...
  for (const book of books) {
    ...
    img.src = safeImageSrc(book.cover && book.cover !== "custom" ? book.cover : "assets/default-cover.jpg");
...
/** 只放行 http/https 绝对地址、相对路径、data:image/* —— 别的一律回默认封面 */
function safeImageSrc(src) {
  const value = String(src || "").trim();
  if (!value) return "assets/default-cover.jpg";
  if (/^https?:\/\//i.test(value)) return value;
  if (/^data:image\//i.test(value)) return value;
  if (/^[a-z0-9._/-]+$/i.test(value)) return value; // 相对路径 assets/xxx.jpg
  return "assets/default-cover.jpg";
}

window.openBook = openBook;
window.addBookPage = addBookPage;
window.refreshShelf = refreshShelf;
