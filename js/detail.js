...
/* 下载口令走 window.DOWNLOAD_PASSWORD getter（在 js/storage.js），
   优先读用户自设的口令，没设的时候用老默认值，改一处两边都生效 */

/* 同 add-book.js：手机上 ♥ 字符会被当彩色 emoji，CSS 上色无效，改用内联 SVG */
const HEART_SVG =
  '<svg class="heart-svg" viewBox="0 0 32 29" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M16 27.9C16 27.9 1.6 19.6 1.6 10.1 1.6 5.4 5.3 1.8 9.8 1.8c2.6 0 4.9 1.2 6.2 3.2 1.3-2 3.6-3.2 6.2-3.2 4.5 0 8.2 3.6 8.2 8.3 0 9.5-14.4 17.8-14.4 17.8z" fill="currentColor"/></svg>';
...
function renderRating() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    heart.innerHTML =
      '<span class="heart-bg">' + HEART_SVG + "</span>" +
      '<span class="heart-fill" style="width:' + percent + '%"><span>' + HEART_SVG + "</span></span>";
...
function renderMeta() {
  ...
  if (currentBook.url) {
    const safeUrl = safeHref(currentBook.url);
    if (safeUrl) {
      box.innerHTML +=
        `<div class="meta-item"><span class="meta-label">网址</span>` +
        `<a class="meta-value link" href="${escapeHtml(safeUrl)}" target="_blank" rel="noopener">打开原页面</a></div>`;
    }
  }
}

/** 只放行 http/https/mailto，防止 javascript: 之类 self-XSS */
function safeHref(url) {
  const value = String(url || "").trim();
  if (!value) return "";
  if (/^(https?|mailto):/i.test(value)) return value;
  if (/^\/\//.test(value)) return "https:" + value;    // 协议相对
  if (/^[^\s:]+$/.test(value)) return "https://" + value; // 用户随手粘的没协议地址
  return "";
}
  ...
  list.forEach((item, index) => {
    ...
    if (item.kind === "link") {
      ...
      row.innerHTML = `
        ...
        ${linkHref
          ? `<a class="mini-btn" href="${escapeHtml(linkHref)}" target="_blank" rel="noopener">打开</a>`
          : `<span class="mini-btn" style="opacity:.5">链接不合法</span>`}
      `;
    } else {
      row.innerHTML = `
